...
def run_one(item):
    ...
    cfg = exp / 'config.json'
    ckpt = exp / 'best.pth'
    out_dir = exp / 'extended_eval_internal_test'
    ...
    if not out_file.exists():
        ...
        cmd = [PY, str(EVAL), '--exp', str(exp), '--batch_size', str(bs), '--workers', '3', '--out_dir', str(out_dir)]
        env = os.environ.copy()
        env['CUDA_VISIBLE_DEVICES'] = str(gpu)
        t0 = time.time()
        print('[RUN] gpu{} {} batch={}'.format(gpu, name, bs), flush=True)
        ret = subprocess.run(cmd, cwd=str(ROOT), env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        log_path = ROOT / 'semantic_launcher_logs' / ('internal_eval_{}.log'.format(name))
        log_path.write_text(ret.stdout, encoding='utf-8')
        if ret.returncode != 0:
            print('[FAIL] {} gpu{} rc={}'.format(name, gpu, ret.returncode), flush=True)
            return {'name': name, 'status': 'failed', 'returncode': ret.returncode, 'log': str(log_path)}
        print('[DONE] gpu{} {} {:.1f}s'.format(gpu, name, time.time() - t0), flush=True)
    try:
...
def write_summary(summary):
    out = ROOT / 'outputs_semantic_ddp' / 'internal_extended_all_experiments_summary_20260518.json'
    out.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding='utf-8')
    md = ROOT / 'outputs_semantic_ddp' / 'internal_extended_all_experiments_summary_20260518.md'
    lines = ['# Internal test extended evaluation summary', '', '| Experiment | Dice | IoU | Precision | Recall | Specificity | Accuracy | MCC | HD95 | ASSD | Boundary F1 |', '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|']
    for r in summary:
        if r.get('status') != 'ok':
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=4) as ex:
        futs = {ex.submit(run_one, item): item[1] for item in enumerate(EXPS)}
        for fut in as_completed(futs):
            results.append(fut.result())
            write_summary(sorted(results, key=lambda r: EXPS.index(r['name']) if r['name'] in EXPS else 999))
    write_summary(sorted(results, key=lambda r: EXPS.index(r['name']) if r['name'] in EXPS else 999))
